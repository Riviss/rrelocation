...
const distmax = 4.0           # maximum catalog(input) distance to join clusters (km)
const distmax2 = 4.0          # maximum relocated distance to join clusters (km)
const hshiftmax = 0.3         # maximum permitted horizontal cluster shifts (km)
const vshiftmax = 4.0         # maximum permitted vertical cluster shifts (km)
...
const boxwid = 4.0                # initial "shrinking-box" width (km)
const nit = 15                   # number of iterations
const irelonorm = 1              # relocation norm (L1 norm=1, L2 norm=2, 3=robust L2)
const tdifmax = Float32(50.)     # maximum differential time value allowed (for error-checking on xcor data input)
const torgdifmax = Float32(20.0) # maximum origin time adjustment (for robustness)
